public class PipeV extends Pipee{

            
    public PipeV(boolean outerReinforcement, boolean innerInsulation, boolean chemicalResistance, int noColour, int plasticGrade, double length, double outerDiameter){
        ...
        if (!outerReinforcement) { //this is not okay
            throw new IllegalArgumentException("Must have outer reinforcement.");
        }
        if (!innerInsulation) {
            throw new IllegalArgumentException("Must have insulation.");
        }
        if (2 > plasticGrade || plasticGrade > 6) {
            throw new IllegalArgumentException("Must have plastic grade between 3 and 5.");
        }
        if (noColour != 2) {
